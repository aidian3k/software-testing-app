...
import static project.eepw.softwaretestingcrud.IntegrationTestConstants.GET_ALL_COMMENTS_URL;
...
import static project.eepw.softwaretestingcrud.IntegrationTestConstants.createLocalURIWithGivenPortNumber;
...
public class CommentBehaviourTests {
  ...
  @Given("There is added user with with email {string} and name {string} and surname {string} and password {string}")
  public void addUserToSystem(String email, String name, String surname, String password) {
    User user = User
        .builder()
        .name(name)
        .surname(surname)
    ...
    backgroundUser = restTemplate.postForEntity(
  ...
  public void addPostToExistingUser(String postContent) {
    ...
    backgroundPost = restTemplate.postForEntity(
        createLocalURIWithGivenPortNumber(port, GET_ALL_POSTS_URL + "/user/" + backgroundUser.getId()),
        postCreationDTO,
  ...
  public void thePostWithIdDoesNotHaveCommentsYet() {
    Post savedPost = restTemplate.getForEntity(
        createLocalURIWithGivenPortNumber(port, GET_ALL_POSTS_URL + "/" + backgroundPost.getId()),
        Post.class).getBody();
  ...
  public void addCommentToExistingPost(String commentContent) {
    ...
    lastCommentResponse = restTemplate.postForEntity(
        createLocalURIWithGivenPortNumber(port, GET_ALL_COMMENTS_URL),
        commentCreationDTO, Comment.class);
  }
  ...
  public void theSystemReturnsTheResponseCodeToTheUser(int responseCode) {
    assertThat(lastCommentResponse.getStatusCode().value()).isEqualTo(responseCode);
  }

  @And("The system returns the comment class with content {string}")
  public void theSystemReturnsTheCommentClassWithIdAndContentSome(String commentContent) {
  ...
  public void theCommentWithIdAndContentExistsInDatabase(String commentContent) {
    CommentCreationDTO commentCreationDTO = CommentCreationDTO
        .builder()
        .content(commentContent)
        .postId(backgroundPost.getId())
  ...
  public void theOtherCommentWithIdAndContentExistsInDatabase(String commentContent) {
    CommentCreationDTO commentCreationDTO = CommentCreationDTO
        .builder()
        .content(commentContent)
        .postId(backgroundPost.getId())
  ...
  public void userWantsToSearchForAllCommentsOfThePost() {
    lastCommentsResponse = restTemplate.getForEntity(
        createLocalURIWithGivenPortNumber(port, GET_ALL_COMMENTS_URL + "/" + backgroundPost.getId()),
        Comment[].class);
  }
  ...
  @And("The list contains comment with content {string} and comment with content {string}")
  public void theListContainsTwoSpecificComments(String commentContent1, String commentContent2) {
    Comment[] comments = lastCommentsResponse.getBody();
    assertThat(comments).extracting("content").containsExactlyInAnyOrder(
        commentContent1,
        commentContent2
    );
  }
  ...
  public void theSystemReturnsResponseCodeToTheUser(int responseCode) {
    assertThat(lastCommentsResponse.getStatusCode().value()).isEqualTo(responseCode);
  }
  ...
  public void thereIsASecondPostWithContent(String postContent) {
    ...
    secondPost = restTemplate.postForEntity(
        createLocalURIWithGivenPortNumber(port, GET_ALL_POSTS_URL + "/user/" + backgroundUser.getId()),
        postCreationDTO,
        PostDTO.class).getBody();
  }
  ...
  public void thePostHasTheCommentWithContent(String commentContent) {
    ...
    savedComment = restTemplate.postForEntity(
        createLocalURIWithGivenPortNumber(port, GET_ALL_COMMENTS_URL),
        commentCreationDTO,
        Comment.class).getBody();
  }

  @When("The user wants to delete the comment with content {string} from the post")
  public void theUserWantsToDeleteTheCommentWithContentFromThePost(String commentContent) {
    assertThat(savedComment.getContent()).isEqualTo(commentContent);

    lastDeleteResponse = restTemplate.exchange(
        createLocalURIWithGivenPortNumber(port, GET_ALL_COMMENTS_URL + "/" + savedComment.getId()),
        HttpMethod.DELETE,
        null,
        Void.class
    );
  ...
  public void theSystemReturnsCode(int responseCode) {
    assertThat(lastDeleteResponse.getStatusCode().value()).isEqualTo(responseCode);
  }
  ...
  @When("User wants to delete not existing comment with random id {int} from the post")
  public void userWantsToDeleteTheCommentWithIdFromThePost(int commentId) {
    try {
      lastDeleteResponse = restTemplate.exchange(
          createLocalURIWithGivenPortNumber(port, GET_ALL_COMMENTS_URL + "/" + commentId),
          HttpMethod.DELETE,
          null,
          Void.class
      );
